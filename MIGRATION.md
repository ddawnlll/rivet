...
| `rivet-core` (HarnessCore) | `packages/core/src/rivet/harness.ts` | Lifecycle / canonical cognitive cycle / state store / session resume | **PORTED** |
...
- [x] **Phase 4**: Canonical cognitive loop ported (`HarnessCore`, `RunPhase`, state transition, observation and verification cycles, `AuthorizedAction` enforcement).
...
- [x] **Phase 8**: Real cognitive episodes & adversarial verification suites implemented and passing:
  - **Episode A (Read-only cognition)**: PASS
  - **Episode B (Restart continuity & Cognitive View rehydration)**: PASS
  - **Episode C (Governed mutation & Praxis verification)**: PASS
  - **Episode D (Adversarial gauntlet: fake VERIFIED, forged evidence ID, stale CAS, directory traversal)**: PASS

## 7. Fork Verification Status

- `bun typecheck` in `packages/core`: **PASS (0 errors)**
- `bun typecheck` in `packages/opencode`: **PASS (0 errors)**
- `bun test test/rivet` in `packages/core`: **PASS (74/74 tests passing in 15 files)**
