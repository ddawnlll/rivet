...
describe("Rivet task control plane P1", () => {
  it.effect("parent obligation verification cannot mint or close recovery verification", () =>
    Effect.gen(function* () {
      const events = yield* EventV2.Service
      const { db } = yield* Database.Service
      const semantics = yield* SessionSemantics.load(db, SessionV2.ID.make("ses_recovery_independence"))
      yield* semantics.ensureGoal(events, "/goal preserve recovery authority", "/tmp/rivet-control", "execution")
      const parent = semantics.hardState.executionFocus!
      const frame = yield* semantics.pushRecovery(events, {
        failureClass: "verification_gap",
        objective: "Restore an independent verifier path",
        acceptanceCriteria: ["Recovery verifier independently passes"],
      })
      const evidenceId = createEvidenceId()
      yield* semantics.append(events, {
        type: "evidence_recorded",
        evidenceId,
        source: "praxis.parent",
        summary: "Parent obligation evidence",
        timestamp: new Date().toISOString(),
      })
      const parentReceipt: VerificationReceipt = {
        receiptId: createReceiptId(),
        obligationId: parent.targetObligationId!,
        passed: true,
        evidenceId,
      ...
      yield* semantics.append(events, {
        ...
        timestamp: parentReceipt.timestamp,
      })
      yield* semantics.settleFocusAfterVerification(events, parentReceipt)

      expect(semantics.hardState.recoveryFrames.get(frame.id)?.status).toBe("open")
      expect(semantics.hardState.executionFocus?.kind).toBe("recovery")
      expect(semantics.hardState.recoveryVerificationReceipts.has(frame.id)).toBe(false)
    }),
  )

  it.effect("recovery-specific Praxis evaluates its own contract and closes only the active recovery", () =>
    Effect.gen(function* () {
      const events = yield* EventV2.Service
      const { db } = yield* Database.Service
      const semantics = yield* SessionSemantics.load(db, SessionV2.ID.make("ses_recovery_verifier_authority"))
      yield* semantics.ensureGoal(events, "/goal verify recovery independently", "/tmp/rivet-control", "execution")
      const parent = semantics.hardState.executionFocus!
      const predicate = semantics.hardState.obligationPredicates.get(parent.targetObligationId!)
      if (predicate?.type !== "claims_verified") throw new Error("Expected claims_verified root predicate")
      const frame = yield* semantics.pushRecovery(events, {
        failureClass: "verification_gap",
        objective: "Restore the declared claims verifier",
        acceptanceCriteria: [`Praxis passes claims_verified: ${predicate.claimPropositions.join("; ")}`],
      })
      const request = {
        obligationId: frame.targetObligationId,
        predicate: "verify recovery contract",
        targetScope: semantics.scope("/tmp/rivet-control"),
        timeoutSeconds: 30,
        timestamp: new Date().toISOString(),
      }

      const failed = yield* semantics.verifyRecovery(events, request)
      ...
      yield* semantics.append(events, {
        type: "evidence_recorded",
        evidenceId,
        source: "file.recovery.fixture",
        summary: predicate.claimPropositions[0]!,
        timestamp: new Date().toISOString(),
      })
      yield* semantics.proposeClaim(events, {
        repository: "/tmp/rivet-control",
        proposition: predicate.claimPropositions[0]!,
        supportingEvidence: [evidenceId],
      })
      const passed = yield* semantics.verifyRecovery(events, request)

      expect(passed.passed).toBe(true)
      expect(passed.acceptanceCriteria).toEqual(frame.acceptanceCriteria)
      ...
      expect(semantics.hardState.verificationReceipts.has(parent.targetObligationId!)).toBe(false)
    }),
  )
